  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl shadow-black/50">
      ...
      <div className="bg-slate-950 border-b border-slate-800 px-6 py-4 flex items-center justify-between">
         <h2 className="text-sm font-bold text-slate-300 tracking-wider uppercase">Active Listings</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-800 bg-slate-950/50 text-xs text-slate-500 font-mono uppercase tracking-wider">
          ...
          <tbody className="divide-y divide-slate-800 text-sm">
            {ipos.map((ipo) => (
              <tr key={ipo.id} className="group hover:bg-slate-800/50 transition-colors cursor-pointer relative">
                ...
                <td className="px-6 py-4 text-right font-mono">
                  <span className={clsx(
                    "font-bold",
                    ipo.gmp > 0 ? "text-emerald-400" : ipo.gmp < 0 ? "text-rose-400" : "text-slate-400"
                  )}>
                ...
                <td className="px-6 py-4 text-slate-400 font-mono text-xs">
                  {ipo.listing_date || <span className="text-slate-600 italic">TBA</span>}
                </td>
                ...
                <td className="px-6 py-4 text-center">
                  <span className={clsx(
                    ...
                    ipo.status === "Open" ? "bg-emerald-500 text-emerald-950 border-emerald-400" :
                    ipo.status === "Closed" ? "bg-slate-700 text-slate-300 border-slate-600" :
                      ...
                      {ipo.trend.length > 0 ? (
                        ipo.trend.map((point, i) => (
                           <div
                             ...
                             className={clsx(
                               "w-1 rounded-t-sm",
                               point.price > 0 ? "bg-emerald-500" : "bg-slate-600"
                             )}
                ...
                {/* Actions */}
                <td className="px-6 py-4 text-right">
                  <Link href={`/ipo/${ipo.id}`} className="p-2 text-slate-500 hover:text-white transition-colors inline-block">
        ...
        {ipos.length === 0 && !loading && (
           <div className="p-12 text-center text-slate-500 font-mono text-sm border-t border-slate-800">
